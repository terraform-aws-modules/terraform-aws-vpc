...
The recomendation for Transit Gateways is to use dedicated subnets for its attachments. By default only one route table will be created and associated to all Transit Gateway attachments (you can disable this behavior via `create_single_transit_gateway_route_table`).

Transit Gateway subnets are considered internal (no direct internet access). However, you can include a default route pointing to the NAT Gateway (if it exists) by using `create_transit_gateway_nat_gateway_route`. This is useful when you want to create a central internet breakdown so that your NAT Gateways can be shared with other VPCs via a transit gateway.

> If `create_transit_gateway_nat_gateway_route` is enabled, `create_single_transit_gateway_route_table` will be ignored. This is done to make sure each transit gateway subnet has a route to its corresponding NAT Gateway.

For Transit Gateway best practices you can read [tgw best design practices](https://docs.aws.amazon.com/vpc/latest/tgw/tgw-best-design-practices.html).
...
| <a name="input_create_single_transit_gateway_route_table"></a> [create\_single\_transit\_gateway\_route\_table](#input\_create\_single\_transit\_gateway\_route\_table) | Controls if one or multiple route tables for transit gateway should be created. If `create_transit_gateway_nat_gateway_route` is set to `true`, this option will be ignored | `bool` | `true` | no |
| <a name="input_create_transit_gateway_nat_gateway_route"></a> [create\_transit\_gateway\_nat\_gateway\_route](#input\_create\_transit\_gateway\_nat\_gateway\_route) | Controls if a nat gateway route should be created to give internet access to the transit gateway subnets | `bool` | `false` | no |
| <a name="input_create_transit_gateway_subnet_route_table"></a> [create\_transit\_gateway\_subnet\_route\_table](#input\_create\_transit\_gateway\_subnet\_route\_table) | Controls if separate route table for transit gateway should be created | `bool` | `true` | no |
...
| <a name="output_transit_gateway_nat_gateway_route_ids"></a> [transit\_gateway\_nat\_gateway\_route\_ids](#output\_transit\_gateway\_nat\_gateway\_route\_ids) | List of IDs of the transit gateway nat gateway route. |
| <a name="output_transit_gateway_network_acl_arn"></a> [transit\_gateway\_network\_acl\_arn](#output\_transit\_gateway\_network\_acl\_arn) | ARN of the transit gateway network ACL |
| <a name="output_transit_gateway_network_acl_id"></a> [transit\_gateway\_network\_acl\_id](#output\_transit\_gateway\_network\_acl\_id) | ID of the transit gateway network ACL |
| <a name="output_transit_gateway_route_table_association_ids"></a> [transit\_gateway\_route\_table\_association\_ids](#output\_transit\_gateway\_route\_table\_association\_ids) | List of IDs of the transit gateway route table association |
| <a name="output_transit_gateway_route_table_ids"></a> [transit\_gateway\_route\_table\_ids](#output\_transit\_gateway\_route\_table\_ids) | List of IDs of transit gateway route tables |
