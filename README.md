...
| [aws_ec2_tag.nat_gateway_eni](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/ec2_tag) | resource |
...
| <a name="input_nat_gateway_eni_tags"></a> [nat\_gateway\_eni\_tags](#input\_nat\_gateway\_eni\_tags) | Additional tags for the network interfaces (ENIs) created by the NAT gateways. Note: applied with the `aws_ec2_tag` resource, so provider `default_tags` are not inherited | `map(string)` | `{}` | no |
