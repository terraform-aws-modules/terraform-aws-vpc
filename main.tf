locals {
  max_subnet_length = max(
    ...
    length(var.eks_private_subnets),
...
resource "aws_route_table" "eks_public" {
  ...
  tags = merge(
    {
      "Name" = format("%s-${var.eks_public_subnet_suffix}", var.name)
    },
    var.tags,
    var.eks_public_route_table_tags,
  )
}
...
resource "aws_route" "eks_public_internet_gateway_ipv6" {
  count = var.create_vpc && var.enable_ipv6 && length(var.eks_public_subnets) > 0 ? 1 : 0
...
resource "aws_route_table" "eks_private" {
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.eks_private_subnet_suffix}" : format(
        "%s-${var.eks_private_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
    var.tags,
    var.eks_private_route_table_tags,
  )
...
resource "aws_subnet" "eks_public" {
  count = var.create_vpc && length(var.eks_public_subnets) > 0 && (false == var.one_nat_gateway_per_az || length(var.eks_public_subnets) >= length(var.azs)) ? length(var.eks_public_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = element(concat(var.eks_public_subnets, [""]), count.index)
  availability_zone               = element(var.azs, count.index)
  map_public_ip_on_launch         = var.map_public_ip_on_launch
  assign_ipv6_address_on_creation = var.eks_public_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.eks_public_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.eks_public_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.eks_public_subnet_ipv6_prefixes[count.index]) : null

  tags = merge(
    {
      "Name" = format(
        "%s-${var.eks_public_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
...
resource "aws_subnet" "eks_private" {
  count = var.create_vpc && length(var.eks_private_subnets) > 0 ? length(var.eks_private_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = var.eks_private_subnets[count.index]
  availability_zone               = element(var.azs, count.index)
  assign_ipv6_address_on_creation = var.eks_private_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.eks_private_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.eks_private_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.eks_private_subnet_ipv6_prefixes[count.index]) : null
...
resource "aws_network_acl" "eks_public" {
  count = var.create_vpc && var.eks_public_dedicated_network_acl && length(var.eks_public_subnets) > 0 ? 1 : 0
...
resource "aws_network_acl_rule" "eks_public_inbound" {
  count = var.create_vpc && var.eks_public_dedicated_network_acl && length(var.eks_public_subnets) > 0 ? length(var.eks_public_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.eks_public[0].id

  egress          = false
  rule_number     = var.eks_public_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.eks_public_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.eks_public_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.eks_public_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.eks_public_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.eks_public_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.eks_public_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.eks_public_inbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.eks_public_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_network_acl_rule" "eks_public_outbound" {
  count = var.create_vpc && var.eks_public_dedicated_network_acl && length(var.eks_public_subnets) > 0 ? length(var.eks_public_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.eks_public[0].id

  egress          = true
  rule_number     = var.eks_public_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.eks_public_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.eks_public_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.eks_public_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.eks_public_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.eks_public_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.eks_public_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.eks_public_outbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.eks_public_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_network_acl" "eks_private" {
  count = var.create_vpc && var.eks_private_dedicated_network_acl && length(var.eks_private_subnets) > 0 ? 1 : 0
  ...
  tags = merge(
    {
      "Name" = format("%s-${var.eks_private_subnet_suffix}", var.name)
    },
    var.tags,
    var.eks_private_acl_tags,
  )
}
...
resource "aws_network_acl_rule" "eks_private_inbound" {
  count = var.create_vpc && var.eks_private_dedicated_network_acl && length(var.eks_private_subnets) > 0 ? length(var.eks_private_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.eks_private[0].id

  egress          = false
  rule_number     = var.eks_private_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.eks_private_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.eks_private_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.eks_private_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.eks_private_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.eks_private_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.eks_private_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.eks_private_inbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.eks_private_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_network_acl_rule" "eks_private_outbound" {
  count = var.create_vpc && var.eks_private_dedicated_network_acl && length(var.eks_private_subnets) > 0 ? length(var.eks_private_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.eks_private[0].id

  egress          = true
  rule_number     = var.eks_private_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.eks_private_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.eks_private_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.eks_private_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.eks_private_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.eks_private_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.eks_private_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.eks_private_outbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.eks_private_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_route_table_association" "eks_public" {
  count = var.create_vpc && length(var.eks_public_subnets) > 0 ? length(var.eks_public_subnets) : 0
...
resource "aws_vpn_gateway_route_propagation" "eks_public" {
  count = var.create_vpc && var.propagate_eks_public_route_tables_vgw && (var.enable_vpn_gateway || var.vpn_gateway_id != "") ? 1 : 0
...
resource "aws_vpn_gateway_route_propagation" "eks_private" {
  count = var.create_vpc && var.propagate_eks_private_route_tables_vgw && (var.enable_vpn_gateway || var.vpn_gateway_id != "") ? length(var.eks_private_subnets) : 0
