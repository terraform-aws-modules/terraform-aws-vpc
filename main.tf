...
resource "aws_network_acl_rule" "public_inbound" {
  for_each = local.create_public_subnets && var.public_dedicated_network_acl ? {
...
resource "aws_network_acl_rule" "outpost_inbound" {
  for_each = local.create_outpost_network_acl ? {
    for rule in var.outpost_inbound_acl_rules : rule["rule_number"] => rule
  } : {}
...
resource "aws_network_acl_rule" "outpost_outbound" {
  for_each = local.create_outpost_network_acl ? {
    for rule in var.outpost_outbound_acl_rules : rule["rule_number"] => rule
  } : {}
