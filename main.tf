...
resource "aws_internet_gateway" "this" {
  count = var.create_vpc && ! var.disable_igw_creation && length(var.public_subnets) > 0 ? 1 : 0
...
resource "aws_route" "public_internet_gateway_ipv6" {
  count = var.create_vpc && ! var.disable_igw_creation && var.enable_ipv6 && length(var.public_subnets) > 0 ? 1 : 0
...
resource "aws_route" "database_internet_gateway" {
  count = var.create_vpc && ! var.disable_igw_creation && var.create_database_subnet_route_table && length(var.database_subnets) > 0 && var.create_database_internet_gateway_route && false == var.create_database_nat_gateway_route ? 1 : 0
